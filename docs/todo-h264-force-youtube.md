...
Forcing H.264 flips this for the broadest device coverage. But note we don't always need H.264
*specifically*: on a device with hardware VP9 (like the Kaby Lake i3 above), stripping **only AV1**
lets YouTube fall back to VP9, which already decodes in hardware there. H.264-forcing is only required
on devices **without** hardware VP9 (the Pi, generic arm64, older x86). This argues for a per-device
strip list rather than a blanket "H.264 only." Trade-off if we do force H.264: YouTube caps it at
1080p (no 4K H.264) and it's slightly less bandwidth-efficient — generally acceptable for a
video/kiosk browser, and far better than dropped frames from software AV1.
